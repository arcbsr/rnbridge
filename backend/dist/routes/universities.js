...
router.get('/', async (req, res) => {
    try {
        const result = await database_1.pool.query(`SELECT * FROM universities 
...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await database_1.pool.query('SELECT * FROM universities WHERE id = $1', [id]);
...
router.get('/country/:country', async (req, res) => {
    ...
    try {
        const result = await database_1.pool.query('SELECT * FROM universities WHERE country = $1 ORDER BY ranking ASC', [country]);
        res.json({
            success: true,
            data: result.rows
        });
...
router.post('/', async (req, res) => {
    const { name, country, city, ranking, tuition_fee_min, tuition_fee_max, programs, requirements, application_deadline, website_url, contact_email } = req.body;
    try {
        ...
        const result = await database_1.pool.query(`INSERT INTO universities (
...
router.put('/:id', async (req, res) => {
    ...
    const { name, country, city, ranking, tuition_fee_min, tuition_fee_max, programs, requirements, application_deadline, website_url, contact_email } = req.body;
    try {
        const result = await database_1.pool.query(`UPDATE universities 
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const result = await database_1.pool.query('DELETE FROM universities WHERE id = $1 RETURNING *', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                error: 'University not found'
            });
...
router.get('/budget/:min/:max', async (req, res) => {
    ...
    try {
       ...
       ORDER BY tuition_fee_min ASC`, [parseFloat(min), parseFloat(max)]);
        res.json({
            success: true,
            data: result.rows
        });
